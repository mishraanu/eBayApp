...
//Method to verify display of element
	public static boolean isElementVisible(findType type, String locator) throws Exception
	{
		...
		try
		{
			...
			logger.info("[Msg-Info] : Checking if an element is visible");
			...
			logger.info("[Msg-Result] Is Displayed : "+genericElement.isDisplayed());
			logger.info("[Msg-Result] Is Enabled : "+genericElement.isEnabled());

		
			if(genericElement.isDisplayed() || genericElement.isEnabled())
			{
				logger.info("[Msg-Success] : Element is visible");
				isVisible = true;
			}
		}
		catch(NoSuchElementException e)
		{
			logger.info("[Msg-Info] : Element is not visible");
	...
	public static void clickOnElement(findType type, String locator) throws Exception
	{
		try
		{
			...
			logger.info("[Msg-Info] : Tapping on element...");
			Thread.sleep(1000);
			switch(type)
			{
				case XPATH:
	...
	public static void enterTextInElement(findType type, String locator, String textToEnter) throws Exception
	{
		...
		try
		{
			...
			logger.info("[Msg-Info] : Entering Text in a text field");
			...
			try
			{
				...
				{
					logger.info("[Msg-Info] : Looks like there is some text present already, clearing that text...");
					genericElement.clear();
				}
			}
			catch(NullPointerException e)
			{
				logger.info("[Msg-Info] : Text field is empty");
			}
			...
			try
			{
				logger.info("Entering text in :"+textToEnter);
				genericElement.sendKeys(textToEnter);
			}
			catch(WebDriverException e)
			{
				logger.info("[Msg-Info] : In catch");
				Thread.sleep(1000);
	...
	public static void swipeOnScreen(String direction) throws Exception
	{
		try
		{
			logger.info("[Msg-Info] : Swiping "+direction+" on the screen");
	...
	public static void swipeOnScreen (int cordX, int cordY , swipeType stype, int offset) throws Exception
	{
		try
		{
			...
			logger.info("[Msg-Info] : Swiping on the screen");	
			switch(stype)
			{
				case BTT:
	...
	public static void swipeOnScreen(findType ftype, String locator, swipeType stype, int offset) throws Exception
	{
		try
		{
			...
			logger.info("[Msg-Info] : Swiping ..."); 

			int x = genericElement.getLocation().getX();
			int y = genericElement.getLocation().getY();
			
			logger.info("x cord - "+x);
			logger.info("y cord - "+y);

			logger.info("[Msg-Info] : Position : "+x+" , "+y);
	...
	public static boolean isTextOfElementCorrect(findType type, String locator, String textToVerify, compareType compType) throws Exception
	{
		...
		try
		{
			...
			WebElement genericElement = null;
			Thread.sleep(100);
			logger.info("[Msg-Info] : Checking if text on an element is correct");
			...
			logger.info("[Displayed] : "+displaydText);
			logger.info("[Expected]  : "+textToVerify.trim());
			
			if(compType == compareType.EQUALS)
			{
				if(displaydText.equals(textToVerify.trim()))
				{
					logger.info("[Msg-Success] : Element is visible with expected Text");
					isVisible = true;
				}
			}
    ...
    public static void hideKeyboardByDownKey() throws Exception
    {
    		try 
    		{
    			logger.info("[Msg-Info] : Hiding keyboard....");
    			driver.hideKeyboard();
    		} 
    		catch (Exception e) 
    		{
    			logger.info( "[Msg-Info] : Not able to hide keyboard");
    			logger.log(Level.WARNING, e.getMessage(), e);
    		}
    }
	...
	public static WebElement productNameInList = null;
		...
		public static boolean isElementVisibleByName(elementOfEbay type, String expText) throws Exception
		{
			...
			try
			{
				...
				logger.info("[Msg-Info] :  Verifying presence of : "+type);
				...
				if(isDisplayed)
					logger.info("[Msg-Info] :  Element "+type+" is visible on screen");
